...
- [X] [Harvard CS50 - Asymptotic Notation (video)](https://www.youtube.com/watch?v=iOq5kSKqeR4)
	- Review of
		- O(1) constant time where no matter the size of the input the time to commpletion is the same
		- O(n) linear time algos where time to complete grows linerally with input
		- O(n^2) exponetntial time algos where time to complete grows exponentially with increased input
		- O(logn) logarithmic time where the larger the input the closer the aglo gets to constant time
		- O(2^n) ya done goofed 

	- Upper and lower bounds for algos
		- O this is the worst case(usually the one we care about)
		- Omega this is the best case(most times its Omega(1))
		- Theta this is when both the upper and lower bounds are the same

- [X] [Big O Notations (general quick tutorial) (video)](https://www.youtube.com/watch?v=V6mKVRU1evU)
	- Examples in Java of the above algorithims 
	- Homework
		- [] write O(1), O(n), O(n^2), O(log n), O(n log n) algos in python
